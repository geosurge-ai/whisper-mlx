...
DAEMON_PORT = 15997  # Use non-standard port to avoid conflicts
...
class TestClient:
    ...
    def post(
        self, path: str, data: dict[str, Any], timeout: float = REQUEST_TIMEOUT
    ) -> HttpResponse:
        """POST request."""
...
class TestProfilesEndpoint:
    ...
    def test_general_profile_has_no_tools(self, client: TestClient) -> None:
        ...
        general: dict[str, Any] | None = next(
            (p for p in body if p.get("name") == "general"), None
        )
    ...
    def test_mirror_profile_has_tools(self, client: TestClient) -> None:
        ...
        mirror: dict[str, Any] | None = next(
            (p for p in body if p.get("name") == "mirror"), None
        )
