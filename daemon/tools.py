    ...
    def load_mirror_tools() -> dict[str, ToolFunction]:
        ...
        try:
            ...
            def make_stub(error: str) -> ToolFunction:
                ...
                return stub
            for name in ["search_linear_issues", "get_linear_issue", "list_linear_events",
                         "search_slack_messages", "get_slack_thread", "lookup_user"]:
...
def _create_browser_tools_loader() -> ToolLoader:
    ...
    def load_browser_tools() -> dict[str, ToolFunction]:
        ...
        try:
            ...
            def make_stub(error: str) -> ToolFunction:
                def stub(**kwargs: Any) -> str:
                    return json.dumps({"error": f"code_runner_agent not available: {error}"})
                return stub
            for name in ["web_search", "browser_navigate", "browser_get_text", "browser_click",
                         "browser_get_elements", "browser_wait", "browser_paste_code",
                         "browser_type_slow", "browser_press_key", "browser_analyze_page"]:
                _cache[name] = make_stub(err_msg)
